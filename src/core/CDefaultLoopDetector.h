/**
 * File: CDefaultLoopDetector.h
 * Date: November 2011
 * Author: Dorian Galvez-Lopez
 * Description: demo application of DLoopDetector
 * License: see the LICENSE.txt file
 *
 * Modified by Peter Califano (petercalifano.gs@gmail.com), September 2025
 */
...
class CDefaultLoopDetector
{
  public:
     ...
     */
    CDefaultLoopDetector(const std::string &vocfile,
    ...
    void run(const std::string &name,
             const CFeatureExtractor<TDescriptor> &extractor);
...
// ---------------------------------------------------------------------------
template <class TVocabulary, class TDetector, class TDescriptor>
CDefaultLoopDetector<TVocabulary, TDetector, TDescriptor>::CDefaultLoopDetector(const std::string &vocfile,
                                                                                const std::string &imagedir,
                                                                                const std::string &posefile,
                                                                                int width,
                                                                                int height,
...
// ---------------------------------------------------------------------------
template <class TVocabulary, class TDetector, class TDescriptor>
void CDefaultLoopDetector<TVocabulary, TDetector, TDescriptor>::run(const std::string &name, const CFeatureExtractor<TDescriptor> &extractor)
{

    std::cout << "DLoopDetector running with: " << name << "\n";
    ...
    cout << "Loading " << name << " vocabulary from file " << m_vocfile << "..." << "\n"; // FIXME this seems to cause the program to remain stuck
    ...
    {
        cout << "Adding image " << i << ": " << filenames[i] << "... " << "\n";
        ...
        else
        {
            ...
            case CLOSE_MATCHES_ONLY:
                cout << "All the images in the database are very recent" << "\n";
                break;
            ...
            case LOW_SCORES:
                cout << "No match reaches the score threshold (alpha: " << params.alpha << ")" << "\n";
                break;

            case NO_GROUPS:
                cout << "Not enough close matches to create groups. "
                     << "Best candidate: " << result.match << "\n";
                break;
    ...
    if (count == 0)
    {
        cout << "No loops found in this image sequence" << "\n";
    }
    else
    {
        cout << count << " loops found in this image sequence!" << "\n";
    }
...
// ---------------------------------------------------------------------------
template <class TVocabulary, class TDetector, class TDescriptor>
void CDefaultLoopDetector<TVocabulary, TDetector, TDescriptor>::readPoseFile(const char *filename, std::vector<double> &xs, std::vector<double> &ys)
    const
{
    xs.clear();
    ys.clear();
